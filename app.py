...
@app.route("/api/orders/clear_all", methods=["DELETE"])
@login_required
def api_clear_all():
    d = request.json or {}
    if d.get("confirm") != "CLEAR":
        return jsonify({"success": False, "error": "ยืนยันไม่ถูกต้อง"}), 400
    try:
        # ลบ items ก่อน แล้วลบ orders
        deleted = Order.query.count()
